...
template <typename Derived>
bool
RecursiveASTVisitor<Derived>::VisitOMPCostClause(OMPCostClause *C) {
  TRY_TO(VisitOMPClauseWithPreInit(C));
  TRY_TO(TraverseStmt(C->getCost()));
  return true;
}

template <typename Derived>
bool
RecursiveASTVisitor<Derived>::VisitOMPLabelClause(OMPLabelClause *C) {
  TRY_TO(VisitOMPClauseWithPreInit(C));
  TRY_TO(TraverseStmt(C->getLabel()));
  return true;
}
