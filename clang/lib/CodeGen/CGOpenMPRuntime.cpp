...
// For ARM & AVX
llvm::Function *CGOpenMPRuntime::emitVReductionFunctionARMAVX(
    SourceLocation Loc, llvm::Type *ArgsType, ArrayRef<const Expr *> Privates,
    ArrayRef<const Expr *> LHSExprs, ArrayRef<const Expr *> RHSExprs,
    ArrayRef<const Expr *> ReductionOps) {
  ASTContext &C = CGM.getContext();
  ...
  // default for SVE is 128 bits
  int VWidth = 1; // Vector Register Width - multiple of 128 bits
  
  if(CGM.getTarget().hasFeature("avx2"))
    VWidth = 2;
  else  if(CGM.getTarget().hasFeature("avx512f"))
    VWidth = 4;
  
  auto VarAddressNThreads = CGF.GetAddrOfLocalVar(VarNThreads);
  auto nThreads =  CGF.Builder.CreateLoad(VarAddressNThreads);

  // emit a vector and a remider loop for each reduction operation
  unsigned I = 0;  
  for (const Expr *E : ReductionOps) {                
    llvm::BasicBlock *ForHead = CGF.createBasicBlock("VLoopHead");
    llvm::BasicBlock *ForBody = CGF.createBasicBlock("VLoopBody");    
    llvm::BasicBlock *ForExit = CGF.createBasicBlock("VLoopExit");     
    llvm::BasicBlock *RemForBody = CGF.createBasicBlock("RemLoopBody");
    llvm::BasicBlock *RemForExit = CGF.createBasicBlock("RemLoopExit");        
    ...
    const auto *RHSArrayVar =
        cast<VarDecl>(cast<DeclRefExpr>(RHSExprs[I])->getDecl()); 
    auto RHSArrayVarAddress = CGF.GetAddrOfLocalVar(RHSArrayVar);
    llvm::Type* SrcElemType = RHSArrayVarAddress.getElementType();
    bool isIntTy = SrcElemType->isIntegerTy();
    uint32_t numElems= getNumElem64bits(SrcElemType)*2*VWidth; 
    auto SrcVecType = llvm::VectorType::get(SrcElemType, numElems, false);
    auto SrcVecPtrType = SrcVecType->getPointerTo(); 
        
    auto nThreadsVLenClipped = CGF.Builder.CreateAnd(nThreads, 
                              llvm::APInt::getSignedMaxValue(32)-(numElems-1));
    ...
    llvm::Value *CondVWidth = 
      CGF.Builder.CreateICmpSLT(nThreads, CGF.Builder.getInt32(numElems));
    CGF.Builder.CreateCondBr(CondVWidth, RemForBody, ForBody);        
    ...
    auto VecGEP = CGF.Builder.CreateGEP(RHSArrayVarAddress.getElementType(), 
                                  RHSArrayVarAddress.getPointer(), IndexVar);
    auto VecGEPCast = CGF.Builder.CreateBitCast(VecGEP, SrcVecPtrType);      
    clang::CodeGen::Address VecAddress(VecGEPCast, 
                                        RHSArrayVarAddress.getAlignment());
    auto VecLoaded = CGF.Builder.CreateLoad(VecAddress);

    llvm::Value *VecHReduced = emitReductionOpInstruction(CGF, PhiVec, 
                                        VecLoaded, isIntTy, ReductionOpCode);
    
    llvm::Value *NextIndexVar = CGF.Builder.CreateAdd(IndexVar, 
                                              CGF.Builder.getInt32(numElems));
    PhiVec->addIncoming(VecHReduced, ForBody);
    IndexVar->addIncoming(NextIndexVar, ForBody);    

    CGF.Builder.CreateCondBr(
                CGF.Builder.CreateICmpEQ( NextIndexVar, nThreadsVLenClipped ),
                ForExit, ForBody);  

    CGF.EmitBlock(ForExit);
    llvm::Value *VLoopReducedValue = emitReductionOpIntrinsic(CGF, VecHReduced,
                                                SrcElemType, ReductionOpCode);    
    CGF.Builder.CreateCondBr(
                  CGF.Builder.CreateICmpEQ( nThreadsVLenClipped, nThreads ), 
                  RemForExit, RemForBody);    
    
    CGF.EmitBlock(RemForBody);
    llvm::PHINode *RemIndexVar = CGF.Builder.CreatePHI(CGM.Int32Ty, 3);
    llvm::PHINode *RemPhiVec = CGF.Builder.CreatePHI(SrcElemType, 3);

    RemIndexVar->addIncoming(CGF.Builder.getInt32(0), ForHead); 
    RemPhiVec->addIncoming(IdentityVal, ForHead);
    RemIndexVar->addIncoming(nThreadsVLenClipped, ForExit); 
    RemPhiVec->addIncoming(VLoopReducedValue, ForExit);

    auto RHSArrayGiEP = CGF.Builder.CreateGEP(
                              RHSArrayVarAddress.getElementType(), 
                              RHSArrayVarAddress.getPointer(), RemIndexVar);
    clang::CodeGen::Address RHSiElemAddress(RHSArrayGiEP, 
                                            RHSArrayVarAddress.getAlignment());
    auto iElemLoaded = CGF.Builder.CreateLoad(RHSiElemAddress);

    llvm::Value *TmpReducedValue = emitReductionOpInstruction(CGF, RemPhiVec, 
                                        iElemLoaded, isIntTy, ReductionOpCode);
    
    llvm::Value *NextIndexVarRem = CGF.Builder.CreateAdd(RemIndexVar, 
                                                    CGF.Builder.getInt32(1));
    RemIndexVar->addIncoming(NextIndexVarRem, RemForBody);    
    RemPhiVec->addIncoming(TmpReducedValue, RemForBody);
        
    CGF.Builder.CreateCondBr(
                    CGF.Builder.CreateICmpEQ(NextIndexVarRem, nThreads), 
                    RemForExit, RemForBody);
    
    CGF.EmitBlock(RemForExit);
    llvm::PHINode *PhiReducedValue = 
              CGF.Builder.CreatePHI( RHSArrayVarAddress.getElementType(), 2);
    PhiReducedValue->addIncoming(VLoopReducedValue, ForExit);
    PhiReducedValue->addIncoming(TmpReducedValue, RemForBody);

    const auto *LHSVar =
          cast<VarDecl>(cast<DeclRefExpr>(LHSExprs[I])->getDecl());
    auto LHSVarAddress = CGF.GetAddrOfLocalVar(LHSVar);
    CGF.Builder.CreateStore(PhiReducedValue, LHSVarAddress);        
...
void CGOpenMPRuntime::emitReduction(CodeGenFunction &CGF, SourceLocation Loc,
  ...
  else if (CGM.getLangOpts().OpenMPUseVBR && 
                    ( ( CGM.getTriple().getArch() == llvm::Triple::aarch64 && 
                        CGM.getTarget().hasFeature("sve") ) || 
                      ( CGM.getTriple().getArch() == llvm::Triple::x86_64 && 
                        CGM.getTarget().hasFeature("avx512f")) ) ) {
    ReductionFn = emitVReductionFunctionARMAVX(
            Loc, CGF.ConvertTypeForMem(ReductionArrayTy)->getPointerTo(), 
            Privates, LHSExprs, RHSExprs, ReductionOps);
  }
  else {
