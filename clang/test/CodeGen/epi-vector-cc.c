// NOTE: Assertions have been autogenerated by utils/update_cc_test_checks.py
// RUN: %clang_cc1 -triple riscv64 -mepi -emit-llvm -o- %s \
// RUN:       | FileCheck %s

void basic_test_callee(__epi_1xf64 a, __epi_1xf64 b, __epi_1xi1 mask);

// CHECK-LABEL: @basic_test_caller(
// CHECK-NEXT:  entry:
// CHECK-NEXT:    [[A:%.*]] = alloca <vscale x 1 x double>, align 8
// CHECK-NEXT:    [[B:%.*]] = alloca <vscale x 1 x double>, align 8
// CHECK-NEXT:    [[MASK:%.*]] = alloca <vscale x 1 x i1>, align 8
// CHECK-NEXT:    [[TMP0:%.*]] = load <vscale x 1 x double>, ptr [[A]], align 8
// CHECK-NEXT:    [[TMP1:%.*]] = load <vscale x 1 x double>, ptr [[B]], align 8
// CHECK-NEXT:    [[TMP2:%.*]] = load <vscale x 1 x i1>, ptr [[MASK]], align 8
// CHECK-NEXT:    call void @basic_test_callee(<vscale x 1 x double> noundef [[TMP0]], <vscale x 1 x double> noundef [[TMP1]], <vscale x 1 x i1> [[TMP2]])
// CHECK-NEXT:    ret void
//
void basic_test_caller(void)
{
  __epi_1xf64 a;
  __epi_1xf64 b;
  __epi_1xi1 mask;

  basic_test_callee(a, b, mask);
}
