...
std::int32_t RTNAME(SelectedRealKind)(
  ...
  // We only support binary for now.
  if (radix == 0)
    radix = 2;
  if (radix != 2)
    return -5;

  auto ExactMatch = std::find_if(
      std::begin(realKinds), std::end(realKinds), [r, p](const RealKinds &rk) {
        return rk.precision >= p && rk.range >= r;
      });
  if (ExactMatch != std::end(realKinds))
    return ExactMatch->kind;

  bool pOK = std::find_if(std::begin(realKinds), std::end(realKinds),
                 [p](const RealKinds &rk) { return rk.precision >= p; }) !=
      std::end(realKinds);
  bool rOK = std::find_if(std::begin(realKinds), std::end(realKinds),
                 [r](const RealKinds &rk) { return rk.range >= r; }) !=
      std::end(realKinds);
