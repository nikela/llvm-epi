...
! CHECK:         %[[VAL_8:.*]] = fir.coordinate_of %[[VAL_1]], %[[VAL_6]] : (!fir.ref<!fir.type<_QMmooTmytype{x:i32}>>, !fir.field) -> !fir.ref<i32>
...
! CHECK:         %[[VAL_10:.*]] = fir.field_index x, !fir.type<_QMmooTmytype{x:i32}>
! CHECK:         %[[VAL_11:.*]] = fir.coordinate_of %[[VAL_2]], %[[VAL_10]] : (!fir.ref<!fir.type<_QMmooTmytype{x:i32}>>, !fir.field) -> !fir.ref<i32>
! CHECK:         %[[VAL_12:.*]] = fir.load %[[VAL_11]] : !fir.ref<i32>
! CHECK:         %[[VAL_13:.*]] = arith.constant 42 : i32
! CHECK:         %[[VAL_14:.*]] = arith.cmpi ne, %[[VAL_12]], %[[VAL_13]] : i32
! CHECK:         cond_br %[[VAL_14]], ^bb1, ^bb2
! CHECK:       ^bb1:
! CHECK:         %[[VAL_15:.*]] = fir.address_of(@_QQcl.696E76616C6964206F7574707574) : !fir.ref<!fir.char<1,14>>
! CHECK:         %[[VAL_16:.*]] = arith.constant 14 : index
! CHECK:         %[[VAL_17:.*]] = fir.convert %[[VAL_15]] : (!fir.ref<!fir.char<1,14>>) -> !fir.ref<i8>
! CHECK:         %[[VAL_18:.*]] = fir.convert %[[VAL_16]] : (index) -> i64
! CHECK:         %[[VAL_19:.*]] = arith.constant false
! CHECK:         %[[VAL_20:.*]] = arith.constant false
! CHECK:         %[[VAL_21:.*]] = fir.call @_FortranAStopStatementText(%[[VAL_17]], %[[VAL_18]], %[[VAL_19]], %[[VAL_20]]) : (!fir.ref<i8>, i64, i1, i1) -> none
! CHECK:         fir.unreachable
! CHECK:       ^bb2:
! CHECK:         return
! CHECK:       }
...
! CHECK-LABEL: func @_QMmooPmyblah() {
