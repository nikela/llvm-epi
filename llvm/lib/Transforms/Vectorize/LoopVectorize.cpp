...
protected:
  ...
  /// update their users.
  void fixFixedOrderRecurrence(VPHeaderPHIRecipe *PhiR,
                               VPTransformState &State,
...
InstructionCost LoopVectorizationCostModel::getVectorCallCost(
  ...
  if (!TLI || CI->isNoBuiltin() || !VecFunc || Legal->isMaskRequired(CI))
    return VF.isScalable() && !Legal->preferPredicatedVectorOps()
...
void InnerLoopVectorizer::fixCrossIterationPHIs(VPTransformState &State) {
  ...
  for (VPRecipeBase &R : Header->phis()) {
    if (auto *ReductionPhi = dyn_cast<VPReductionPHIRecipe>(&R))
      ...
      fixFixedOrderRecurrence(FOR, State, /* Predicated */ true);
    }
  }
}

void InnerLoopVectorizer::fixFixedOrderRecurrence(
    VPHeaderPHIRecipe *PhiR, VPTransformState &State,
    bool Predicated) {
...
bool LoopVectorizationCostModel::isPredicatedInst(Instruction *I) const {
  ...
  case Instruction::Call:
    return Legal->isMaskRequired(I) && !Legal->preferPredicatedVectorOps();
  }
}
...
void LoopVectorizationCostModel::collectLoopUniforms(ElementCount VF) {
  ...
  // vectorizetion, because that would create an erroneous replicating region
...
LoopVectorizationCostModel::computeFeasibleMaxVFScalableOnly(
  ...
  ElementCount FeasibleMaxVFLowerBound = ElementCount::getFixed(0);
  ElementCount FeasibleMaxVFUpperBound = ElementCount::getFixed(0);
...
LoopVectorizationCostModel::getInstructionCost(Instruction *I, ElementCount VF,
  ...
  case Instruction::Call: {
    ...
    CallCost = getVectorCallCost(CI, VF, &Variant);
...
VPValue *VPRecipeBuilder::getOrCreateIV(VPBasicBlock *VPBB, VPlan &Plan) {
...
VPWidenEVLRecipe *VPRecipeBuilder::getOrCreateEVL(VPlan &Plan) {
  if (EVLRecipe)
    return EVLRecipe;
...
VPRecipeBase *VPRecipeBuilder::tryToWidenCallCommon(
    CallInst *CI, ArrayRef<VPValue *> Operands, VFRange &Range,
    VPlanPtr &Plan,
    bool VPPredicated) {
  ...
  if (VPPredicated) {
    ...
    if (VPID != Intrinsic::not_intrinsic ||
      ...
      return new VPPredicatedWidenCallRecipe(
          *CI, make_range(Ops.begin(), Ops.end()), VPID,
          createBlockInMask(CI->getParent(), *Plan), getOrCreateEVL(*Plan));
  }
...
VPRecipeBase *VPRecipeBuilder::tryToWidenCall(CallInst *CI,
                                              ArrayRef<VPValue *> Operands,
                                              VFRange &Range, VPlanPtr &Plan) {
  return tryToWidenCallCommon(CI, Operands, Range, Plan,
                              /* Predicated */ false);
}

VPRecipeBase *
VPRecipeBuilder::tryToPredicatedWidenCall(CallInst *CI,
                                          ArrayRef<VPValue *> Operands,
                                          VFRange &Range, VPlanPtr &Plan) {
  return tryToWidenCallCommon(CI, Operands, Range, Plan, /* Predicated */ true);
}
...
void VPRecipeBuilder::fixHeaderPhis(VPlanPtr &Plan) {
  ...
  VPEVLPHIRecipe *EVLPhiRecipe = nullptr;
  for (VPHeaderPHIRecipe *R : PhisToFix) {
    if (auto *PredPhi =
            dyn_cast<VPPredicatedFirstOrderRecurrencePHIRecipe>(R)) {
      if (!EVLPhiRecipe) {
...
VPRecipeBuilder::tryToCreateWidenRecipe(Instruction *Instr,
  ...
  if (auto Phi = dyn_cast<PHINode>(Instr)) {
    ...
    if (Legal->isReductionVariable(Phi)) {
      ...
      if (preferPredicatedWiden()) {
        PhiRecipe = new VPPredicatedFirstOrderRecurrencePHIRecipe(Phi, *StartV);
      } else {
  ...
  if (auto *CI = dyn_cast<CallInst>(Instr)) {
    if (preferPredicatedWiden())
      return toVPRecipeResult(
          tryToPredicatedWidenCall(CI, Operands, Range, Plan));
...
VPlanPtr LoopVectorizationPlanner::buildVPlanWithVPRecipes(
  ...
  addCanonicalIVRecipes(*Plan, Legal->getWidestInductionType(),
                        ...
                        CM.getTailFoldingStyle(IVUpdateMayOverflow), EVLRecipe);
  ...
  VPlanTransforms::adjustFixedOrderRecurrences(*Plan, Builder, EVLRecipe);
    ...
    for (Instruction *R : ReductionOperations) {
      ...
      if (RecurrenceDescriptor::isMinMaxRecurrenceKind(Kind)) {
        assert((isa<VPWidenSelectRecipe>(WidenRecipe) ||
                isa<VPPredicatedWidenSelectRecipe>(WidenRecipe)) &&
               "Expected to replace a VPWidenSelectSC or "
...
void VPWidenMemoryInstructionRecipe::execute(VPTransformState &State) {
  ...
  // FIXME: We only use it for the context, I'm sure we can get it from
  // other places.
  // FIXME: We should be using VPBuilder here!
  BasicBlock *VectorPH = State.CFG.getPreheaderBBFor(this);
    ...
    for (unsigned Part = 0; Part < State.UF; ++Part) {
      ...
      if (CreateGatherScatter) {
        ...
        if (EVLPart) {
          VectorType *StoredValTy = cast<VectorType>(StoredVal->getType());
          Function *VPIntr = Intrinsic::getDeclaration(
              VectorPH->getModule(), Intrinsic::vp_store,
