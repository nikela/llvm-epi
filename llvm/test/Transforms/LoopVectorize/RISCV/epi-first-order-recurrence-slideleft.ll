...
; RUN:    -loop-vectorize -vector-register-width-factor=8 -instcombine -force-vector-interleave=1 < %s  -o - | FileCheck %s
; RUN: opt -mtriple riscv64 -mattr +m,+a,+f,+d,+experimental-v -S -dce -riscv-v-vector-bits-min=64 \
; RUN:    -loop-vectorize -instcombine -force-vector-interleave=1 < %s  -o - | FileCheck %s --check-prefix=CHECK1
; RUN: opt -mtriple riscv64 -mattr +m,+a,+f,+d,+experimental-v -S -dce -riscv-v-vector-bits-min=64 \
