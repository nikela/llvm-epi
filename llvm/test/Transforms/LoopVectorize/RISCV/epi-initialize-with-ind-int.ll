...
; RUN:    -scalable-vectorization=only -loop-vectorize -vector-register-width-factor=8 -force-vector-interleave=1 < %s -o - | FileCheck %s
; RUN: opt -mtriple riscv64 -mattr +m,+a,+f,+d,+epi,+experimental-v -S -riscv-v-vector-bits-min=64 \
; RUN:    -scalable-vectorization=only -loop-vectorize -force-vector-interleave=1 < %s -o - | FileCheck %s --check-prefix=CHECK1
; ModuleID = 't2.i'
