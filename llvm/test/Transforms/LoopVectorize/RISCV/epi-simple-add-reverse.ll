...
; RUN:    -loop-vectorize -vector-register-width-factor=8 -instcombine -riscv-v-vector-bits-min=64 | FileCheck %s
; RUN: opt < %s -mtriple riscv64 -mattr +m,+a,+f,+d,+epi,+experimental-v -S -dce \
; RUN:    -loop-vectorize -instcombine -riscv-v-vector-bits-min=64 | FileCheck %s --check-prefix=CHECK1
